...
public class Task implements Runnable {
	...
	private SURFIPointList InterestPoints;
...
//				}
				...
				if((buffer.head == imageNum) && surf.finishDetection) {
...
//		}
	...
	public void runDescriptionTask() {
		int[] res = buffer.myPoll(descriptPackageNum);
		if(res != null) {
			for(int i = res[1]; i < res[1] + res[2]; i++) {	//[head, index, number]
				getOrientation(buffer.get(res[0]).get(i), buffer.get(res[0]).get(i).mIntegralImage);
				getMDescriptor(buffer.get(res[0]).get(i), true, buffer.get(res[0]).get(i).mIntegralImage);
//				System.out.println("picNo=" + res[0] + " i=" + i + " - " + buffer.get(res[0]).get(i).getOrientation());
			}
		}
	}
