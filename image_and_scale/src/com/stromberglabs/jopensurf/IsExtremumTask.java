...
public class IsExtremumTask implements Runnable {
	...
	public IsExtremumTask(ResponseLayer bb, ResponseLayer mm, ResponseLayer tt, FastHessian f, CountDownLatch l,  LinkedBlockingQueue<SURFInterestPoint> q, IntegralImage image, int getDescriptionThreadNumber, boolean getIPointConcurrency) {
		b = bb;
		m = mm;
		t = tt;
		fh = f;
...
//			System.out.println("getPointConcurrency=true");
			...
			for (int i = 0; i < threadNum; i++)
				exec.execute(new IPointsDescriptionProcessor(ipointsQueue, true, this, ipointNum, latch, false, null));
			try {
				latch.await();
			} catch (InterruptedException e) {
